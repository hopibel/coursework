 ...
 * <br>
 * Note: Used to visualize the game tree during testing only. Please don't include this class
 * during grading because it is not my work. - Hope Ibelgaufts
 */
public class TreeView extends JComponent {
    ...
    HashMap<MCTSNode, Color> high;

    public TreeView(MCTSNode root) {
        this.root = root;
        high = new HashMap<MCTSNode, Color>();
    }
    ...
    private void draw(Graphics2D g, MCTSNode cur, int x, int y, int wFac) {
        // draw this one, then it's children

        int arity = cur.arity();
        for (int i = 0; i < arity; i++) {
            if (cur.children.get(i).visits > 0) {
    ...
    private void drawNode(Graphics2D g, MCTSNode node, int x, int y) {
        String s = (int) node.totalValue + "/" + (int) node.visits;
        g.setColor(nodeBg);
